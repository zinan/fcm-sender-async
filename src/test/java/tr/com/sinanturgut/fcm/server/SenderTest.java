...
import static tr.com.sinanturgut.fcm.server.util.FixtureHelpers.fixture;
import static tr.com.sinanturgut.fcm.server.util.JsonHelpers.jsonFixture;
...
public class SenderTest {
  ...
  public void testSuccess() throws InterruptedException, ExecutionException, TimeoutException, IOException {
    ...
    Sender                   sender  = new Sender("foobarbaz", 10, server.getUrl("/fcm/send").toExternalForm());
    ...
    assertEquals(request.getPath(), "/fcm/send");
  ...
  public void testBadApiKey() throws ExecutionException, InterruptedException, TimeoutException {
    ...
    Sender                   sender = new Sender("foobar", 10, server.getUrl("/fcm/send").toExternalForm());
  ...
  public void testServerError() throws TimeoutException, InterruptedException {
    ...
    Sender sender = new Sender("foobarbaz", 2, server.getUrl("/fcm/send").toExternalForm());
    ListenableFuture<Result> future = sender.send(Message.newBuilder().withDestination("1").build());
  ...
  public void testServerErrorRecovery() throws InterruptedException, ExecutionException, TimeoutException {
    ...
    Sender sender = new Sender("foobarbaz", 3, server.getUrl("/fcm/send").toExternalForm());
    ListenableFuture<Result> future = sender.send(Message.newBuilder().withDestination("1").build());
